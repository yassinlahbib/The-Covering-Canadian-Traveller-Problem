...
class CNN():
    ...
    def blocked_edges(self, i):
        ...
        for j in range (self.n):
            if tuple(sorted((i, j))) in self.cctp_graph.blocked_edges:
                ...
                debug_print(f"Blocked edge: {i} -> {j}")
    ...
    def shortcut(self):
        ...
        while j < self.n :
            ...
            debug_print(f"P[i]={self.P[i]} -> Blocked edges: {self.blocked_edges(self.P[i])}")
            if (self.P[i], self.P[j]) not in Eb:
                P_prime.append(self.P[j]) # On ajoute la ville j à P'
                i = j
            else :
                Us.add(self.P[j]) # On ajoute la ville j à Us
                debug_print(f"Blocked edge: {Us=}")
            j = j+1
        debug_print(f"nombre de blocked edges: {len(Eb)}")
    ...
    def compress(self, Us, P_prime):
        ...
        for i in range (self.n):
            if i not in Us:
                debug_print(f"i={i} not in {Us}")
                G_prime[i, :] = np.inf #On enleve les sommets visités
                G_prime[:, i] = np.inf 
        debug_print("APRES")
        debug_print(f"{G_prime=}")
    ...
    def algo_exploration(self, G):
        ...
        while True:
            debug_print(f"G={G}")
            debug_print(f"current={current}")
            if np.all(G == np.inf):
                debug_print("Aucun sommet n'est atteignable")
                if current == self.s:
                    debug_print("On à fini d'explorer tous les sommets avant d'entrer dans la fonction, c'est a dire Us contient seulement la ville de depart", )
                    return path
                break
            ...
            debug_print(f"voisin={voisin}")
            if (current, voisin) in self.shortest_path:
                debug_print(f"chemin entre {current} et {voisin} : {self.shortest_path[current, voisin][1:]} de distance {G[current][voisin]}")
                ...
                debug_print(f"arete directe entre {current} et {voisin} de distance {G[current][voisin]}")
            G[current,:] = np.inf #On enlève le sommet courant pour ne pas le visiter à nouveau
            G[:,current] = np.inf
            current = voisin
            
            if current == self.s:
                debug_print("On est revenu au sommet de départ")
                break
        debug_print("On a fini d'explorer tous les sommets")
        debug_print(f"Chemin exploré: {path}")
        ...
        if self.shortest_path.get((current, self.s)) is not None:
            ...
            debug_print(f"chemin entre {current} et {self.s} : {self.shortest_path[current, self.s][1:]}")
        else:
            path.append(self.s)
            debug_print(f"arete directe entre {current} et {self.s} de distance {to_initial_state[current]}")
        debug_print(f"Chemin exploré: {path}")
        return path
...
if __name__ == "__main__":
	...
	debug_print(f"Number of cities: {nbCities}")

	graph = CCTPGraph("instances/kroA100.tsp", k=8)
	tsp_path = graph.christophides_solver()
	debug_print("Distance totale (Christofides) :", evaluation(tsp_path, graph.cities))
	debug_print("Chemin TSP (Christofides) :", tsp_path)
	...
	debug_print("Distance totale (CNN) :", evaluation(chemin_final, graph.cities))
	debug_print("Chemin final (CNN) :", chemin_final)